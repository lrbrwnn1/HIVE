...
def LSI(request):
	if request.method == 'GET':
		...
		tl = terms_list.objects.filter(term__icontains=LSI_search_query)#Search term lookup
		tv = term_vectors.objects.all()
		termVectorQS = tv.filter(pk__in=tl).values()#match searched terms to vectors
...
def profile(request):
	if request.method == 'GET':
		profiles = Investigator.objects.all()
		return render(request, 'searchFunction/userprofile.html',{'profiles' : profiles})
